...
function inspectPages(pending, opts) {
  if (pending.length > 0) {
    ...
    const lighthouseConfig = createLighthouseConfig(
      expectations,
      lighthouseParams.config
    );

    console.log(`Running lighthouse test on ${url}`);
    launchChromeAndRunLighthouse(
      url,
      lighthouseParams.flags,
      lighthouseConfig
    ).then(results => {
...
module.exports = configPath => {
  ...
  if (pages.length === 0) {
    console.error(
      "At least one URL must be specified in the pages property of the config JSON file."
    );
    return;
  }
